...
\subsection{Teorema 1:} Si $A$ es un conjunto finito con $n$ elementos, entonces $P(A)$ tiene $2^n$ elementos (la demostración se realiza en el apartado "10 Principio de inducción").
...
Sea $y \in \text{Codom}(f) = \mathbb{R}$ arbitrario, queremos demostrar que existe $x \in \text{Dom}(f) = \mathbb{R}$ tal que $f(x) = y$
...
Como $y \ge -1$, entonces $x = \sqrt{y + 1}$ está bien definido en $\left[0, \infty \right)$, implica que para cualquier $y \in \left[-1, \infty \right)$ existe un único (ya demostramos que $f$ es inyectiva) $x \in \left[0, \infty \right)$
...
\end{axis}
\end{tikzpicture}
\caption{Gráfica de la función y su inversa.}
\end{figure}
